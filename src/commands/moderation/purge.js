...
module.exports = new Command(async (msg, suffix) => {
  ...
  if (isNaN(amount)) return msg.channel.createMessage('Your last argument must be a number')
  if (amount > 100 || amount < 2) return msg.channel.createMessage("You're trying to remove too many/few messages")
  const messages = (await msg.channel.getMessages(200, msg.id)).filter(x => new Date(msg.timestamp) - new Date(x.timestamp) < 1209600000)
  if (messages.length < 2) return msg.channel.createMessage('I was not able to find any messages for purging that are under two weeks old')
