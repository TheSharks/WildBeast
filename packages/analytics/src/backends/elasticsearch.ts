...
export class ElasticsearchLogsBackend implements LogsBackend {
  ...
  async write(logs: LogEntry[]): Promise<void> {
    ...
    if (!this.schemaEnsured) {
      try {
        ...
        container.logger.warn('Failed to ensure logs schema, continuing with write:', error);  
  ...
  private async ensureILMPolicy(): Promise<void> {
    try {
      ...
      container.logger.info(`Created ILM policy: ${policyName} with ${retentionDays} days retention`);
