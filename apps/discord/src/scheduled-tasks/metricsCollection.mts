...
export class MetricsCollectionTask extends ScheduledTask {
  ...
  private updateBotMetrics(analytics: AnalyticsClient, shardLabels: { shard_id: string }) {
    ...
    memoryGauge.set(process.memoryUsage().heapUsed, { ...shardLabels, type: "heap_used" });
    memoryGauge.set(process.memoryUsage().heapTotal, { ...shardLabels, type: "heap_total" });
    memoryGauge.set(process.memoryUsage().rss, { ...shardLabels, type: "rss" });
  }

  private updateDiscordMetrics(analytics: AnalyticsClient, shardLabels: { shard_id: string }) {
  ...
  private updateSystemMetrics(analytics: AnalyticsClient, shardLabels: { shard_id: string }) {
