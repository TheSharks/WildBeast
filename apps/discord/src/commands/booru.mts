...
export class BooruCommand extends AppSubcommand {
  ...
  public override async autocompleteRun(
    ...
    if (!site || !isBooruSiteName(site)) return interaction.respond([])
    // Complete the word being typed; keep the entered tags in front.
    const chunks = interaction.options.getFocused().split(' ')
    const term = chunks.pop() ?? ''
    if (term.length === 0) return interaction.respond([])
