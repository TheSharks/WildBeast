...
- Added clustering: shards can be split across multiple processes, configured with `WILDBEAST_SHARDING_START`, `WILDBEAST_SHARDING_END`, and `WILDBEAST_SHARDING_TOTAL`.
- Added scheduled jobs, starting with an hourly job that posts guild counts to discord.bots.gg.
- Added the `/meme` command.
- Added Gelbooru, Rule34, and Derpibooru to the `/booru` command.
- The randomize button on paginated results stays enabled after the end of the list.

## 8.0.0

### Major Changes

- **This is a breaking change.** WildBeast is rewritten in TypeScript on top of `detritus-client`, replacing Eris. Node.js 14 or newer is required.
- All commands are now slash commands. Prefix commands are removed, along with the `BOT_PREFIX` setting.
- Added support for context menu actions, buttons, and select menus. The first context menu action shows a user's avatar, and long results are paginated with buttons.
- PostgreSQL is the only supported database, configured with `DATABASE_URL`. SQLite and knex are removed, and schema changes run through the new `migrations:up` and `migrations:down` scripts.
- Tags are rebuilt as `/tag create`, `/tag edit`, `/tag delete`, and `/tag show`, stored in PostgreSQL.
- Available commands: `8ball`, `advice`, `booru` (e621, with tag autocomplete), `cat`, `dice`, `dog`, `info`, `inspire`, `invite`, `ping`, `tag`, and `urbandictionary`.
- Removed the music, moderation, and Twitch commands, and `eval`, `say`, `fact`, `jpeg`, `leetspeak`, `stroke`, and `xkcd`.
- Docker images are published to the GitHub Container Registry instead of GitHub Packages.
- Sentry reporting now includes PostgreSQL query tracing.
...
- Security release with updated dependencies, including `js-yaml` and `lodash`. Updating is highly recommended. Fully compatible with previous 6.2.x releases.
- `settings` now ignores uppercase in setting names ([#355](https://github.com/TheSharks/WildBeast/issues/355)).
- Fixed the `info` command and a duplicated `setlevel` entry in command usage.
- Changed the invite URL.

## 6.2.0

_Reconstructed from the git history; no release notes were published for this version._

### Minor Changes

- **Breaking for Rancher deployments:** autoscaling now targets Kubernetes StatefulSets instead of Rancher Cattle.
- Added the `jpeg` command.
- Merged `hackban` into `ban`.
- Renamed the translation string `PERMISSIONS_MALFORMED` to `COMMAND_SYNTAX_INVALID`.
- Stopped using `url.parse`, which Node.js 11 deprecates.
- Fixed welcome messages, shard error handling, skipping when using the ffmpeg encoder, and an error on voice disconnects ([#380](https://github.com/TheSharks/WildBeast/issues/380)).
