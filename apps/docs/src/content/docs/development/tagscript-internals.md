...
| `runtime/renderer.ts` | The render loop: walks the AST, calls handlers. |
| `runtime/registry.ts`, `default-registry.ts` | Tag name to handler mapping. |
| `runtime/limits.ts` | Default limits and the per-pass checks. |
...
`render` is a fixpoint loop: parse the string, render every node, and if
the result differs from the input, do it again. The loop ends when a pass
changes nothing, or throws when `maxIterations` (default 100) is reached
while the output is still changing. This is what makes composition work: a
variable whose value contains tags renders in the next pass.
...
`maxDepth` is enforced during the walk and `maxOutputLength` after every
pass, so runaway nesting or expansion fails fast rather than at the end.

### Inert output

Tags listed in `inertHandlerOutput` (default: `fetch`, `js`, `javascript`)
must not have their output re-executed as TagScript; a fetched response
containing `{js:...}` would otherwise run on the next pass. Backslash
escaping can't express this, because the next pass would consume the
escapes. Instead the renderer swaps `{`, `}`, and `|` in inert output for
private-use Unicode codepoints (`U+E000` to `U+E002`), which nothing
parses, and restores the real characters once, at the very end.
